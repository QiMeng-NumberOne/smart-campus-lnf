...
**说明：**
- 登录页支持微信一键登录与游客模式浏览，用于快速进入系统。
- 首页提供“寻物启事 / 失物招领”双列表展示，并支持关键词检索、分类筛选和快速入口跳转。
- 首页卡片展示核心信息：标题、地点、时间、状态、匹配标签，便于快速浏览与决策。

**用法：**
1. 打开小程序后先在登录页完成身份进入。
2. 在首页输入关键词或点击分类标签进行筛选。
3. 点击卡片进入详情页查看图片、联系方式和评论互动。

<p align="left">
  <img src="./show_images/登录页面展示.png" alt="登录页面展示" width="23%" />
  <img src="./show_images/首页搜索筛选展示.png" alt="首页搜索筛选展示" width="23%" />
...
**说明：**
- 发布页支持普通物品与证件类物品两种模式，支持多图上传、AI分类建议、OCR信息提取。
- 证件类采用“先 OCR 再展示字段”的动态交互方式，仅显示识别到的字段，减少误填负担。
- 证件图片默认走脱敏流程：除姓名外其余字段优先打码，同时敏感文本加密存储。

**用法：**
1. 进入发布页并选择“寻物”或“招领”。
2. 上传图片后可点击“AI识别”获取分类建议。
3. 若为证件类，点击“证件OCR自动提取”，核对后提交。
4. 可点击“清空内容”一键恢复初始表单。

<p align="left">
  <img src="./show_images/发布页面展示.png" alt="发布页面展示" width="48%" />
  <img src="./show_images/发布页面_选择上传身份证_识别加文字提取展示.png" alt="发布页面_选择上传身份证_识别加文字提取展示" width="48%" />
</p>

### 文图搜与消息

**说明：**
- 智能搜索页支持文搜与图搜两种模式，基于 CLIP 相似度返回 Top-K 结果。
- 支持“全部 / 只看寻物 / 只看招领”筛选，便于按业务场景精准检索。
- 消息页聚合系统通知、评论回复、匹配提醒等内容，形成闭环反馈。

**用法：**
1. 进入“文图搜”后选择文搜或图搜模式。
2. 输入文本描述或上传待检索图片。
3. 切换“只看寻物 / 只看招领”进行结果过滤。
4. 在消息页查看通知并跳转到对应详情页处理。

<p align="left">
  <img src="./show_images/文图搜_文搜展示.png" alt="文图搜_文搜展示" width="32%" />
  <img src="./show_images/文图搜_图搜展示.png" alt="文图搜_图搜展示" width="32%" />
  <img src="./show_images/消息展示.png" alt="消息展示" width="32%" />
</p>

### 个人中心

**说明：**
- 个人中心用于查看与维护个人资料、我的发布、我的收藏等。
- 支持资料编辑入口与账户基础信息展示，便于长期使用管理。

**用法：**
1. 点击底部导航“我的”进入个人中心。
2. 进入资料编辑页完善姓名、学院、专业、年级等信息。
3. 在“我的发布”中管理状态（已找回/已认领等）。

<p align="left">
  <img src="./show_images/我的展示.png" alt="我的展示" width="32%" />
</p>
